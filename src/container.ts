...
import { PrismaConnection } from '@/infrastructure/database/prisma.connection'
...
import { PrismaAuditLogRepository } from '@/modules/audit/prisma-audit-log.repository'
...
import { PrismaModelRepository } from '@/modules/models/prisma-model.repository'
...
export const buildContainer = (config: AppConfig = buildConfig()): Container => {
  ...
  // --- Infrastructure -------------------------------------------------------
  const database: IDatabaseConnection = new PrismaConnection(config.database, logger)
  ...
  // --- Repositories -----------------------------------------------------------
  // Every repository shares the single `PrismaClient` connection pool owned by
  // `database`, so a service that needs to write across repositories in one
  // transaction can do so via `database.client.$transaction(...)`.
  const userRepository: IUserRepository = new PrismaUserRepository(database.client)
  const otpRepository: IOtpRepository = new PrismaOtpRepository(database.client)
  const sessionRepository: ISessionRepository = new PrismaSessionRepository(database.client)
  const auditLogRepository = new PrismaAuditLogRepository(database.client)
  const modelRepository: IModelRepository = new PrismaModelRepository(database.client)
  const apiKeyRepository: IApiKeyRepository = new PrismaApiKeyRepository(database.client)
